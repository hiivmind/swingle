...
| `< /dev/null` needed | **Yes — mandatory.** pi reads stdin data when the pipe is open; an infinite stdin causes a `RangeError: Invalid string length` crash. See the pack's verification log and #71. |
...
- **`< /dev/null` is mandatory** — pi reads stdin data when the fd is open;
  leaving stdin attached to a live pipe (e.g. piped from another process) causes a
  `RangeError: Invalid string length` crash. Always redirect stdin to `/dev/null`; see the
