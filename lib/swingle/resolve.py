...
def run_resolve(root, rows_by_id, role_arg, provider, project, excluded):
    role = role_arg.lower(); roles = parse_roles(root); tier_lane = next((value for key, value in roles.items() if role in key), None)
