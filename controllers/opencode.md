...
| --- | --- |
| Skill load | Real `skill` tool. Skills register under their **bare frontmatter `name`** — there is no plugin namespace, so it is `skill(sdd)` / `skill(delegate)` / `skill(swingle-verify)`, and superpowers SDD is `skill(subagent-driven-development)`. If the tool refuses, check `permission.skill` in `opencode.json` (`"*": "allow"` by default; `deny` hides a skill entirely, `ask` prompts). |
