...
def main(default_root):
    ...
    which.add_argument("provider", nargs="?")
    init = sub.add_parser("init")
    init.add_argument("provider", nargs="?")
    dest = init.add_mutually_exclusive_group(required=True)
    dest.add_argument("--project")
    dest.add_argument("--user", action="store_true")
    ...
    root = Path(a.root)
    providers = sorted(
        d.name for d in (root / "providers").glob("*/") if (d / "pack.md").exists()
    )
    ...
    for provider in targets:
        ...
        if dest_path.exists() and not a.force:
            print(f"{dest_path} exists — pass --force to overwrite", file=sys.stderr)
            return 1
        ...
        if argv:
            print(
                f"open catalog: align rows with the live list — run: {' '.join(argv)}"
            )
    return 0
