...
def check_repo_docs(root, manifests):
    ...
    plugin_version = (
        json.loads(plugin.read_text()).get("version") if plugin.exists() else None
    )
    codex_version = (
        json.loads(codex_plugin.read_text()).get("version")
        if codex_plugin.exists()
        else None
    )
    match = (
        re.search(r"\*\*Version:\*\*\s*([0-9.]+)", readme.read_text())
        if readme.exists()
        else None
    )
    if plugin_version and match and plugin_version != match.group(1):
        find(
            f"version mismatch: plugin.json {plugin_version} != README {match.group(1)}"
        )
    if plugin_version and codex_version and plugin_version != codex_version:
        find(
            f"version mismatch: .claude-plugin {plugin_version} != .codex-plugin {codex_version}"
        )
    for pack in (
        ...
        closing = next(
            (i for i, line in enumerate(pack_lines[1:], 1) if line == "---"), None
        )
        if closing is not None and any(
            line.strip() for line in pack_lines[closing + 1 :]
        ):
        ...
        if vdir.is_dir():
            for entry in sorted(vdir.iterdir()):
                if not VERSION_FILE_RE.fullmatch(entry.name):
                    find(
                        f"{entry}: versions/ entries must be <dotted-numeric-version>.md"
                    )
                    continue
                if not entry.is_file() or entry.is_symlink():
                    find(f"{entry}: registry file must be a regular file")
                    continue
                if isinstance(verified_version, str) and VER_RE.fullmatch(
                    verified_version
                ):
                    width = max(
                        len(version_key(entry.stem)), len(version_key(verified_version))
                    )
                    if version_cmp_key(entry.stem, width) > version_cmp_key(
                        verified_version, width
                    ):
                        find(
                            f"{entry}: registry key must not exceed manifest verified-version {verified_version}"
                        )
                try:
                ...
                if current == entry and (
                    header.group("vcli") != manifest.get("cli")
                    or header.group("vver") != verified_version
                ):
                    find(
                        f"{entry}: current registry file must carry a Verified header naming the manifest cli and version"
                    )
        log_dir = pack / "log"
        ...
        shards = []
        for e in entries:
            if e.is_file() and not e.is_symlink() and SHARD_FILE_RE.fullmatch(e.name):
                shards.append(e)
            else:
                find(f"{e}: log/ entries must be YYYY-MM.md regular files")
        if not shards:
        ...
        for shard in shards:
            ...
            for n, line in enumerate(shard.read_text().splitlines(), 1):
                ...
                if not m:
                    find(
                        f"{shard}:{n}: shard entry heading must open with its date (## YYYY-MM-DD …)"
                    )
                    continue
                d = m.group(1)
                try:
                    ...
                    find(f"{shard}:{n}: {d} is not a real calendar date")
                    continue
                if not d.startswith(month):
                    find(f"{shard}:{n}: entry date {d} outside the shard's month")
                if prev and d < prev:
                    find(
                        f"{shard}:{n}: shard entries must be nondecreasing by date ({d} after {prev})"
                    )
                prev = d
    ...
    for pack in (
        ...
        if (pack / "models.md").exists():
            targets.append(
                (pack / "models.md", (pack / "models.md").read_text().splitlines(), 1)
            )
        vdir = pack / "versions"
        if vdir.is_dir():
        ...
        for path, lines, start in targets:
            for n, line in enumerate(lines, start):
                for needle, label in hygiene:
                    if needle in line:
                        find(f"{path}:{n}: pack-hygiene: {label}: {line.strip()[:60]}")
                if ver_claim.search(line):
                    find(
                        f"{path}:{n}: pack-hygiene: body version claim: {line.strip()[:60]}"
                    )
    banned = re.compile(
        r"gpt-5\.6|gemini-3|opencode-go/|deepseek|minimax|qwen|glm-5|Agent tool|TodoWrite|CLAUDE_PLUGIN_ROOT|spawn_agent"
    )
    for directory in ("core", "contracts"):
        for path in (
            sorted((root / directory).glob("*.md"))
            ...
            for n, line in enumerate(path.read_text().splitlines(), 1):
                if (
                    banned.search(line)
                    and "routing" not in line
                    and "precedence" not in line
                ):
                    find(f"{path}:{n}: purity violation: {line.strip()[:70]}")
    ...
    def heading_slugs(text):
        ...
        for line in text.splitlines():
            ...
            if m:
                slugs.add(
                    re.sub(r"[^\w\s-]", "", m.group(1).strip().lower()).replace(
                        " ", "-"
                    )
                )
        return slugs
    ...
    def slugs_for(p):
        if p not in slug_cache:
            slug_cache[p] = heading_slugs(p.read_text()) if p.exists() else None
        return slug_cache[p]
    ...
    def scanned(p):
        ...
        return not (
            "archive/" in rel
            or re.match(r"providers/[^/]+/versions/", rel)
            or ".git" in p.parts
            or ".superpowers" in p.parts
            or ".swingle" in p.parts
            or ".sdd-dispatch" in p.parts
        )
    ...
    for path in sorted(root.rglob("*.md")):
        ...
        for n, line in enumerate(path.read_text().splitlines(), 1):
            for target in link_re.findall(line):
                ...
                if pathpart and not dest.exists():
                    find(f"{path}:{n}: broken link {target}")
                    continue
                ...
                if anchor and dest.suffix == ".md" and scanned(dest):
                    ...
                    if known is not None and anchor not in known:
                        find(
                            f"{path}:{n}: broken anchor #{anchor} → {pathpart or path.name}"
                        )
