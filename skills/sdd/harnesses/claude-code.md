...
The lever alias “all Claude” means `native-subagents` — the in-harness Agent tool, **not**
the `claude` provider pack. Both now exist under this harness, so keep them distinct:

- **“all Claude” / `native-subagents`** → Agent tool, no external process, no model
  resolution. The default for in-harness Claude work.
- **“via claude”** → the external `claude` provider pack (`providers/claude/`), a real
  ...
  when isolation from this session is the point (a clean sub-context, cross-harness parity).
