...
| prompt and workspace transport | pass the prompt as the `--print` argument and the project as `--add-dir "$PROJECT"`; close stdin with `/dev/null` in the result-only shape | this is the current successful transport and does not claim that stdin closure alone fixes a provider hang | approved invocation smoke (2026-08-24) |
...
```bash
agy --print "$(cat "$PROMPT")" --model "$MODEL" --effort "$EFFORT" --output-format json --print-timeout 180s --mode plan --add-dir "$PROJECT" < /dev/null > "$ARTIFACT"
```
