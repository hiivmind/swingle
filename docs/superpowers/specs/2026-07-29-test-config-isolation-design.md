...
Both subprocess helpers use this environment:
...
invokes plain `run`. It must expect the built-in model layer and `config-layer=none`.
The test fails against the current helper because ambient state leaks into the subprocess.
