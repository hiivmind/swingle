...
from .environment import detect_installed_providers, check_provider_version, check_provider_readiness
...
OUTCOME_PREFIXES = {
    "STOP:", "ASK:", "CHANNEL:", "warning:", "native-subagents:",
    "installed:", "active:", "provider:", "layer:", "model:",
    "ready:", "available (auth unverified):",
}


def run_step0(manifests, root, path_dir, lever, task_provider, config_arg, role_arg, project, excluded):
    ...
    if lever == "native-subagents":
        ...
        if strict:
            ...
            for provider in active:
                ...
                rc, output, actual_ver = check_provider_version(manifests[provider], path_dir, timeout)
            ...
            if dropped: print(f"warning: incompatible providers removed: {' '.join(sorted(dropped))}")
            active = [provider for provider in active if provider not in dropped]
        if not active: find("ASK: no active providers")
        else:
            print(f"active: {' '.join(active)}"); role_tier_lane = None
            if role_arg:
                roles = parse_roles(root); role_tier_lane = next((value for key, value in roles.items() if role_arg.lower() in key), None)
                if not role_tier_lane: find(f"STOP: unknown role: {role_arg}")
            lane = role_tier_lane[1] if role_tier_lane else None
            chosen = task_provider or lever or (cfg.get("providers_by_lane", {}).get(lane) if lane else None) or cfg.get("default_provider") or ("codex" if "codex" in active else (active[0] if len(active) == 1 else None))
            if chosen and chosen not in active: find(f"ASK: routed provider inactive: {chosen}")
                ...
                if role_tier_lane:
                    layer, layer_path, layer_rows = resolve_models(chosen, root, project)
                ...
                if not strict:
                    ...
                    rc, output, actual_ver = check_provider_version(manifests[chosen], path_dir, timeout)
                    verified_ver = manifests[chosen].get("verified-version")
                    if rc != 0 or not actual_ver or actual_ver != verified_ver:
                        actual = actual_ver if actual_ver else "unparseable"
                        print(f"warning: incompatible: {chosen} ({actual} != {verified_ver})")
                rc, _ = check_provider_readiness(manifests[chosen], path_dir, int(manifests[chosen].get("readiness-timeout-seconds", 30)))
                if rc != 0:
