...
| prompt and project transport | preserve the complete authored briefing in `$PROMPT` and pass it as the native `"@$PROMPT"` file argument; select the session directory explicitly under `$REPO_ROOT` | the file-prompt syntax preserves all authored bytes including trailing newlines | `pi --help`; approved invocation shape (2026-08-24) |
...
```bash
pi -p --mode json --model "$MODEL" --thinking "$EFFORT" --session-dir "$REPO_ROOT/.swingle-session" "@$PROMPT" < /dev/null > "$ARTIFACT"
```
