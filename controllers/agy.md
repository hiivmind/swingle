# Antigravity controller adapter
...
| Dispatch permission | A controlling agy session runs target CLIs through `run_command`, which is gated by the agy permission baseline (`~/.gemini/antigravity-cli/settings.json`). **Every target CLI agy dispatches to needs its own `command(<cli>)` allow rule** — this is a one-time setup step *before the first drive*, not per-dispatch. Add the rules for the provider CLIs you actually drive to: `command(claude)`, `command(codex)`, `command(opencode)`, `command(grok)`, `command(pi)` (agy driving agy needs none). A target CLI missing from the allow-list auto-denies (silent no-op, exit 0, zero work); the controller's evidence gate catches the empty result, but the dispatch is simply lost until the rule is added. This is the same permission model agy-as-target uses for its own tools; see its provider body (the current registry file), “Headless permission baseline.” |
...
and round-trip its completion marker. Evidence: `providers/agy/log/`.
