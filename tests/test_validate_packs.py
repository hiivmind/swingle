...
def health_line(
    ...
    return (
        f"{p}: installed={installed} version={version} verified={verified} "
...
def test_main_is_reentrant(tmp_path, monkeypatch):
    ...
    with contextlib.redirect_stdout(out):
        monkeypatch.setattr(
            sys, "argv", ["validate-packs", "--root", str(FIX / "bad-missing-p1")]
        )
        assert validate_packs_main() == 1
    out2 = io.StringIO()
    with contextlib.redirect_stdout(out2):
        monkeypatch.setattr(
            sys, "argv", ["validate-packs", "--root", str(FIX / "good-lanes")]
        )
...
def test_resolve_exact_lane_beats_any():
    r = run(
        "--root", str(FIX / "good-lanes"), "--resolve", "per-task reviewer", "alpha"
    )
...
def test_config_unknown_provider_ids_fail_closed():
    r = run(
        ...
        str(FIX / "good-lanes"),
        "--check-config",
        str(FIX / "config-unknown-provider.json"),
    )
    assert r.returncode == 1
...
def test_step0_strict_removal_is_warning_when_route_remains():
    r = run(
        ...
        str(FIX / "good-lanes"),
        "--path-dir",
        str(FIX / "bins-alpha-oldver"),
        "--config",
        str(FIX / "config-require-version.json"),
    )
    assert (
        r.returncode == 1
        and "warning: incompatible providers removed: alpha" in r.stdout
        and "ASK: no active providers" in r.stdout
    )
...
def test_step0_strict_removal_warns_and_routes_to_the_remaining_provider():
    ...
    warning_lines = [
        line for line in r.stdout.splitlines() if line.startswith("warning:")
    ]
...
def test_step0_invalid_manifest_never_detects_or_executes_provider_argv(tmp_path):
    ...
    shutil.copytree(FIX / "bad-interpreter-cli", root)
...
def test_version_probe_rejects_a_suffixed_raw_version_token(tmp_path):
    ...
    alpha.chmod(0o755)
    manifest = packs.parse_front_matter(
        FIX / "good-lanes" / "providers" / "alpha" / "pack.md"
    )
...
def test_list_verified_version_is_finding(tmp_path):
    root = tmp_path / "root"
    shutil.copytree(FIX / "good-yaml", root)
...
def test_symlinked_current_file_is_finding(tmp_path):
    ...
    external.write_text("frozen body\n")
...
def test_symlinked_historical_file_is_finding(tmp_path):
    ...
    pack = root / "providers" / "alpha"
    external = tmp_path / "external.md"
    external.write_text("historical body\n")
...
def test_versions_dir_exempt_from_link_scan(tmp_path):
    root = tmp_path / "vlinks"
    shutil.copytree(FIX / "good-lanes", root)
    vdir = root / "providers" / "alpha" / "versions"
...
def test_empty_registry_file_fails_closed():
    ...
    assert (
        r.returncode == 1 and "registry file must open with a class header" in r.stdout
    )
...
def test_current_registry_must_be_verified_for_manifest_cli_and_version():
    ...
    assert (
        r.returncode == 1
        and "current registry file must carry a Verified header naming the manifest cli and version"
        in r.stdout
    )
...
def test_registry_key_above_manifest_stamp_fails():
    ...
    assert (
        r.returncode == 1
        and "registry key must not exceed manifest verified-version" in r.stdout
    )
...
def test_missing_log_shard_fails():
    ...
    assert (
        r.returncode == 1
        and "log/ must exist with at least one YYYY-MM.md shard" in r.stdout
    )
...
def test_bad_log_shard_filename_fails():
    ...
    assert (
        r.returncode == 1
        and "log/ entries must be YYYY-MM.md regular files" in r.stdout
    )
...
def test_log_shard_entries_must_be_nondecreasing():
    ...
    assert (
        r.returncode == 1 and "shard entries must be nondecreasing by date" in r.stdout
    )
...
def test_log_shard_entry_requires_date_heading():
    ...
    assert (
        r.returncode == 1 and "shard entry heading must open with its date" in r.stdout
    )
...
def test_yaml_eligible_md_row_guard(tmp_path):
    ...
    root = tmp_path / "drift"
    _sh.copytree(FIX / "good-yaml", root)
...
def test_resolve_default_layer_line():
    r = run_env(
        "--root", str(FIX / "good-yaml"), "--resolve", "per-task reviewer", "alpha"
    )
    assert r.returncode == 0
...
def test_version_probe_timeout_never_scrapes(monkeypatch):
    monkeypatch.setattr(
        environment, "run_argv", lambda argv, pd, t: (-2, "timed out after 0.5 seconds")
    )
...
def test_version_probe_oserror_never_scrapes(monkeypatch):
    monkeypatch.setattr(
        environment, "run_argv", lambda argv, pd, t: (-1, "Errno 2 no such file 1.2.3")
    )
...
def test_version_probe_garbage_output(monkeypatch):
    monkeypatch.setattr(
        environment, "run_argv", lambda argv, pd, t: (0, "no digits here")
    )
    assert environment.check_provider_version(FM, [], 1)[2] is None
...
def test_version_probe_extracts_dotted(monkeypatch):
    monkeypatch.setattr(
        environment, "run_argv", lambda argv, pd, t: (0, "tool v2.3.4 (build x)")
    )
...
def test_readiness_status_mapping(monkeypatch):
    for rc_in, status in ((0, "ok"), (-2, "timeout"), (1, "fail"), (-1, "fail")):
        monkeypatch.setattr(
            environment, "run_argv", lambda argv, pd, t, r=rc_in: (r, "")
        )
...
def run_models(*args, **env):
    return subprocess.run(
        [sys.executable, str(SDD_MODELS), *args],
        capture_output=True,
        text=True,
        env=isolated_env(**env),
    )
...
def test_sdd_models_init_project_seeds_and_refuses_overwrite(tmp_path):
    ...
    proj.mkdir()
    r = run_models(
        "init", "alpha", "--root", str(FIX / "good-yaml"), "--project", str(proj)
    )
    assert r.returncode == 0
...
def test_codex_manifest_version_mismatch_fails(tmp_path):
    ...
    (root / ".codex-plugin" / "plugin.json").write_text(
        json.dumps({"version": "1.9.9"})
    )
    (root / "README.md").write_text("**Version:** 2.0.0\n")
    r = run("--root", str(root))
    assert (
        r.returncode == 1 and ".claude-plugin 2.0.0 != .codex-plugin 1.9.9" in r.stdout
    )
...
def test_health_installed_and_uninstalled_inprocess(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("SWINGLE_MODELS", raising=False)
    monkeypatch.setattr(
        environment, "is_provider_installed", lambda fm, pd: fm["id"] == "alpha"
    )
    monkeypatch.setattr(
        environment,
        "run_argv",
        lambda argv, pd, t: (0, "1.0.0" if "--version" in argv else "ready"),
    )
    out = io.StringIO()
    with contextlib.redirect_stdout(out):
        monkeypatch.setattr(
            sys, "argv", ["validate-packs", "--health", "--root", str(root)]
        )
        assert validate_packs_main() == 0
    assert (
        health_line("alpha", installed="yes", version="1.0.0", readiness="ok")
        in out.getvalue()
    )
...
def test_health_version_drift_inprocess(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        environment,
        "run_argv",
        lambda argv, pd, t: (0, "2.0.0" if "--version" in argv else "ready"),
    )
    out = io.StringIO()
    with contextlib.redirect_stdout(out):
        monkeypatch.setattr(
    ...
    assert (
        health_line(
            "alpha", installed="yes", version="2.0.0", drift="yes", readiness="ok"
        )
        in out.getvalue()
    )
...
def test_health_readiness_fail_inprocess(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        environment,
        "run_argv",
        lambda argv, pd, t: (0, "1.0.0") if "--version" in argv else (1, ""),
    )
    out = io.StringIO()
    with contextlib.redirect_stdout(out):
        monkeypatch.setattr(
        ...
        assert validate_packs_main() == 0
    assert (
        health_line("alpha", installed="yes", version="1.0.0", readiness="fail")
        in out.getvalue()
    )
...
def test_health_readiness_timeout(tmp_path, monkeypatch):
    ...
    assert exit_code == 0
    assert (
        health_line("alpha", installed="yes", version="1.0.0", readiness="timeout")
        in output
    )
...
def test_health_version_probe_timeout_reports_no_version(tmp_path, monkeypatch):
    ...
    assert exit_code == 0
    assert (
        health_line("alpha", installed="yes", drift="yes", readiness="timeout")
        in out.getvalue()
    )
...
def test_health_config_layers(tmp_path):
    ...
    # env-unreadable
    r = run_env(
        "--health", "--root", str(root), SWINGLE_CONFIG=str(tmp_path / "missing.json")
    )
...
def test_run_ignores_ambient_swingle_config(tmp_path, monkeypatch):
    ...
    ambient_models = tmp_path / "ambient-models"
    ambient_models.mkdir()
...
def test_health_composes_with_check_config(tmp_path):
    ...
    cfg.write_text('{"disable": ["unknown-provider"]}')
...
def test_health_never_exits_nonzero_for_env_states(tmp_path):
    ...
    r = run_env(
        ...
        "--path-dir",
        str(bin_dir),
        SWINGLE_CONFIG=str(tmp_path / "missing.json"),
    )
    assert r.returncode == 0
    assert (
        health_line(
            "alpha", installed="yes", version="9.9.9", drift="yes", readiness="fail"
        )
        in r.stdout
    )
...
def test_config_superpowers_block_accepted():
    r = run("--check-config", str(FIX / "config-superpowers-good.json"))
    assert r.returncode == 0, r.stdout
...
def test_config_superpowers_unknown_provider_fails():
    r = run(
        ...
        str(FIX / "good-lanes"),
        "--check-config",
        str(FIX / "config-superpowers-ghost.json"),
    )
