...
| --- | --- | --- | --- |
| result-only headless dispatch | pipe the complete authored `$PROMPT` into `omp -p`, pass `$MODEL`, `--thinking "$EFFORT"`, JSON mode, no-tools/no-session as required, set `--cwd "$PROJECT"`, and capture to absolute `$ARTIFACT` | OMP's print mode consumes piped prompt content and its model, thinking, mode, session, tool, and cwd controls are separate | `omp --help`; approved invocation smoke (2026-08-24) |
| prompt, stdin, and workspace transport | use `cat "$PROMPT" | omp ... -p`; do not pass `@file` as the only prompt transport in a non-PTY process | the direct file reference waited, while the pipe closed naturally and completed | approved invocation smoke (2026-08-24) |
...
```bash
cat "$PROMPT" | omp --model "$MODEL" --thinking "$EFFORT" --mode json --no-tools --no-session --cwd "$PROJECT" -p > "$ARTIFACT"
```
