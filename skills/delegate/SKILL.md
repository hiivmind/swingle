...
13. Resolve effort together with the model as one joined choice at dispatch time: inspect current `--help` for how this CLI accepts effort alongside the model — a separate flag, folded into the model identifier, a config-override mechanism, or none. Precedence: an effort the user or task states wins; otherwise the selected model preference's joined effort when the entry carries one; otherwise tier intent — roughly minimal for `cheapest`, moderate for `standard`, high for `most-capable`; otherwise the CLI default — and never invent a top-of-scale level (`xhigh`, `max`, `ultra`) nobody asked for. Grounding reports supported values and exact syntax; the choice stays in this thread. Set what the CLI exposes, and never carry one provider's pattern to another.
