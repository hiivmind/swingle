...
def is_provider_installed(fm, path_dirs):
    ...
    if not cli:
        return False
    return any(
        (Path(directory) / cli).exists() and os.access(Path(directory) / cli, os.X_OK)
        for directory in get_path_dirs(path_dirs)
    )
...
def detect_installed_providers(manifests, path_dirs):
    return [
        provider
        for provider, fm in manifests.items()
        if is_provider_installed(fm, path_dirs)
    ]
...
def run_health(manifests, path_dirs, root, project, scoped_providers, config_arg):
    ...
    if scoped_providers:
        ...
        target_providers = [p for p in scoped_providers if p in manifests] + [
            p
            for p in sorted(manifests.keys())
            if p in scoped_set and p not in scoped_providers
        ]
    ...
    for provider_id in target_providers:
        ...
        if not installed:
            ...
            rc, output, actual_ver = check_provider_version(
                fm, path_dirs, HEALTH_PROBE_TIMEOUT_SECONDS
            )
            if actual_ver:
                ver_str = actual_ver
            ...
            ready_rc, ready_status = check_provider_readiness(
                fm, path_dirs, HEALTH_PROBE_TIMEOUT_SECONDS
            )
        ...
        print(
            f"{provider_id}: installed={inst_str} version={ver_str} verified={verified_ver} drift={drift_str} readiness={readiness_str} registry-layer={reg_layer_str}"
        )
...
def run_argv(argv, path_dirs, timeout):
    try:
        env_path = (
            os.pathsep.join(path_dirs) if path_dirs else os.environ.get("PATH", "")
        )
