...
How the harnesses authenticate, what that means for CI, and the honest position on
subscription-seat economics. The README carries the per-harness CI consequence line; the
detail lives here.

## Authentication per harness
...
An **OAuth-only** harness needs a human-seeded credential store, so it does not run in
headless CI or ephemeral runners as-is. An **API-key-capable** harness (Claude, Grok) can,
with the key supplied as a CI secret. Check which mode the harnesses you depend on use before
wiring them into a pipeline — the mode is per harness, not universal.
...
- **Degradation.** If a provider closes seat-based CLI use, the harnesses whose CLI also
  ...
  `models.yaml` only picks *which* model. Harnesses with no API-key mode lose that route — so
...
and adjudicates rather than silently falling back a tier or to another harness — see
