...
| prompt and stdin transport | pass the complete authored `$PROMPT` through native stdin and use `-C "$REPO_ROOT"` for the working directory; stdin closes at EOF | Codex reads the prompt from stdin without shell command substitution, preserving trailing newline bytes | `codex exec --help`; approved invocation smoke (2026-08-24) |
...
```bash
codex exec --json --ephemeral --skip-git-repo-check -m "$MODEL" -c "model_reasoning_effort=\"$EFFORT\"" -s read-only -C "$REPO_ROOT" < "$PROMPT" > "$ARTIFACT"
```
